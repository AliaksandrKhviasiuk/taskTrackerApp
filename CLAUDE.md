...
- **Never trust an agent's claim that a diff or report was "pasted above" or "included in this message."** An agent's own transcript can silently truncate, and this happened repeatedly in Sprint 4 (Dev-agent and Unit-Tests-agent both did it on multiple tickets). Before passing a diff, test file, or report to the next stage — or merging — verify its actual content directly via `git diff`/`git show`/`Read` in the worktree, not by re-reading the dispatching agent's own summary of what it did.
...
Don't rely on session memory alone to track when the window is due — Sprint 4 ran one calibration check after 4 tickets, then merged 5 more back-to-back with none, discovered only at sprint close-out. `.claude/pipeline-metrics.md` keeps a running "tickets merged since last calibration" counter right above the Calibration checks table for exactly this reason — update it every time you append a per-ticket row, and treat it hitting 4-6 as a hard trigger, not a reminder to eyeball.
...
- **Duplicate forwards new fields by default:** when a story adds a new optional `TaskItem` field, `duplicateTask(id:)` should forward it from source to duplicate unless the ticket explicitly says otherwise. Established convention since KAN-20 (`dueDate`), KAN-30 (`priority`), and KAN-33 (`tag`) — not scope creep to re-litigate per ticket.
