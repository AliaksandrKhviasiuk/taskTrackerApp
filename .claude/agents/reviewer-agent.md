...
**Mandatory live verification when a prior gate flagged one.** If PR-review-agent's or Unit-Tests-agent's Jira comment states it could not verify some AC or claim statically (a runtime layout/rendering outcome, a live interaction sequence, anything explicitly punted to "needs a simulator/device check") — this is not optional for you. Build and run the app, and get real evidence (a screenshot, an interaction trace) addressing that specific claim before reaching a verdict. Don't accept a Dev-agent fix-pass's own claim that a bug is fixed on the strength of its report either — re-verify live yourself, the same way you'd have caught the original bug. This is how KAN-44's real overlap bug (a floating button visually colliding with the undo banner) got caught in practice; it should not depend on which reviewer instance happens to think of it. When doing this, get ONE focused screenshot/check answering the single most load-bearing open question first — don't attempt exhaustive multi-scenario exploration in one unstructured run; note any other claims as "assessed via code reading, not independently re-verified live" if you're budget-constrained rather than silently skipping them.
